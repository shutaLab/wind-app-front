...
export const getNotes = async () => {
  const { data } = await axios.get<Note[]>(
    "https://windap-api.vercel.app/api/windNote"
  );
  return data;
};
...
export const showNote = async (id: number) => {
  const { data } = await axios.get<Note>(
    `https://windap-api.vercel.app/api/windNote/${id}`
  );
  return data;
};
...
export const getFavorites = async (id: number) => {
  const { data } = await axios.get<Favorite[]>(
    `https://windap-api.vercel.app/api/windNote/${id}/favorites`
  );
  return data;
};
...
export const checkFavorite = async (id: number) => {
  const { data } = await axios.get<Favorite>(
    `https://windap-api.vercel.app/api/windNote/${id}/favorite`
  );
  return data;
};
